...
export function ZoneMotion({ zoneIndex }: ZoneProps) {
  ...
  const copyCellKeyDown = useCallback((e: React.KeyboardEvent, value: string, key: string) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      void copyCell(value, key);
    }
  }, []);
  ...
  return (
    ...
    >
      <div className={`${layoutStyles["zone-label"]} ambient-label`}>{t("zone.motion")}</div>
      ...
      <section className={sharedStyles["theme-lab-section"]} aria-labelledby="section-scroll-trigger">
        ...
        >
          <div className={styles["scroll-trigger-content"]}>
            <span className={styles["scroll-trigger-label"]}>{t("motion.scroll.label")}</span>
        ...
        <div className={styles["parallax-scene"]}>
          ...
          <Parallax yRange={[-10, 10]} className={styles["parallax-mid-layer"]}>
            <div className={styles["parallax-mid-card"]}>
              <span className={styles["parallax-mid-label"]}>{t("motion.parallax.mid")}</span>
            </div>
          </Parallax>
          <div className={styles["parallax-fg-layer"]}>
            <span className={styles["parallax-fg-label"]}>{t("motion.parallax.fg")}</span>
          </div>
        </div>
      </section>
      ...
      <section className={sharedStyles["theme-lab-section"]} aria-labelledby="section-morphing-blob">
        ...
        <div className={sharedStyles["section-caption"]} style={{ marginTop: 0, marginBottom: 12 }}>
          {t("motion.blob.caption")}
        </div>
        <MorphingBlob count={3} opacity={0.4} className={styles["blob-container"]}>
          <div className={styles["blob-content"]}>
            <span className={styles["blob-label"]}>{t("motion.blob.label")}</span>
            <span className={styles["blob-sub"]}>{t("motion.blob.sub")}</span>
          </div>
        </MorphingBlob>
      </section>
      ...
      <section className={sharedStyles["theme-lab-section"]} aria-labelledby="section-cursor">
        ...
        <div className={sharedStyles["section-caption"]} style={{ marginTop: 0, marginBottom: 12 }}>
          {t("motion.cursor.caption")}
        </div>
        <CustomCursor shape="circle" size={28} trail trailLength={6} className={styles["cursor-demo"]}>
          <div className={styles["cursor-demo-inner"]}>
            <span className={styles["cursor-demo-label"]}>{t("motion.cursor.label")}</span>
            <span className={styles["cursor-demo-sub"]}>{t("motion.cursor.sub")}</span>
          </div>
        </CustomCursor>
      </section>
      ...
      <section className={sharedStyles["theme-lab-section"]} aria-labelledby="section-layout-indicator">
        ...
        <div className={sharedStyles["section-caption"]} style={{ marginTop: 0, marginBottom: 12 }}>
          {t("motion.layoutIndicator.caption")}
        </div>
      ...
      <section className={sharedStyles["theme-lab-section"]} aria-labelledby="section-page-transition">
        ...
        <div className={sharedStyles["section-caption"]} style={{ marginTop: 0, marginBottom: 12 }}>
          {t("motion.pageTransition.caption")}
        </div>
        ...
        <div className={styles["transition-preview-stage"]}>
          <PageTransition key={transitionKey} type={selectedTransition} className={styles["transition-preview-inner"]}>
            ...
            <span className={styles["transition-preview-sub"]}>
              {selectedTransition === "fade" && t("motion.pageTransition.fade")}
              {selectedTransition === "slide" && t("motion.pageTransition.slide")}
              {selectedTransition === "scale" && t("motion.pageTransition.scale")}
            </span>
          </PageTransition>
        </div>
      </section>
      ...
      <section className={sharedStyles["theme-lab-section"]} aria-labelledby="section-motion-contracts">
        ...
        <div className={styles["motion-table-scroll"]}>
        <table className={styles["motion-table"]}>
          <thead>
            <tr>
              <th scope="col">{t("motion.contracts.intent")}</th>
              <th scope="col">{t("motion.contracts.duration")}</th>
              <th scope="col">{t("motion.contracts.easing")}</th>
              <th scope="col">{t("motion.contracts.distance")}</th>
              <th scope="col">{t("motion.contracts.scale")}</th>
            </tr>
          </thead>
          <tbody>
            {MOTION_INTENTS.map((intent) => {
              ...
              const durationKey = `${intent}:duration`;
              const easingKey = `${intent}:easing`;
              const distanceKey = `${intent}:distance`;
              const scaleKey = `${intent}:scale`;
              return (
                <tr key={intent}>
                  <td style={{ fontWeight: 600 }}>{intent}</td>
                  <td
                    className={`${styles["motion-copy-cell"]} ${copiedValue === durationKey ? styles["motion-copy-cell-copied"] : ""}`}
                    onClick={() => void copyCell(durationLabel, durationKey)}
                    onKeyDown={(e) => copyCellKeyDown(e, durationLabel, durationKey)}
                    tabIndex={0}
                    role="button"
                    title={`Copy ${durationLabel}`}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div className={styles["motion-bar"]} style={{ transform: `scaleX(${c.durationMs / maxDur})` }} />
                      <span>{durationLabel}</span>
                    </div>
                  </td>
                  <td
                    className={`${styles["motion-copy-cell"]} ${copiedValue === easingKey ? styles["motion-copy-cell-copied"] : ""}`}
                    style={{ fontSize: 11, opacity: 0.7, fontFamily: theme.typography.en.mono }}
                    onClick={() => void copyCell(c.easing, easingKey)}
                    onKeyDown={(e) => copyCellKeyDown(e, c.easing, easingKey)}
                    tabIndex={0}
                    role="button"
                    title={`Copy ${c.easing}`}
                  >
                  ...
                  <td
                    className={`${styles["motion-copy-cell"]} ${copiedValue === distanceKey ? styles["motion-copy-cell-copied"] : ""}`}
                    onClick={() => void copyCell(distanceLabel, distanceKey)}
                    onKeyDown={(e) => copyCellKeyDown(e, distanceLabel, distanceKey)}
                    tabIndex={0}
                    role="button"
                  ...
                  <td
                    className={`${styles["motion-copy-cell"]} ${copiedValue === scaleKey ? styles["motion-copy-cell-copied"] : ""}`}
                    onClick={() => void copyCell(scaleLabel, scaleKey)}
                    onKeyDown={(e) => copyCellKeyDown(e, scaleLabel, scaleKey)}
                    tabIndex={0}
                    role="button"
        ...
        <div className={sharedStyles["section-caption"]}>
          Mode: {motionSnap.effectiveMode} | {t("motion.contracts.enterLabel")}: {theme.motion.enterMs}ms | {t("motion.contracts.easingLabel")}: {theme.motion.easing}
        </div>
      </section>
    </section>
  );
...
function LayoutIndicatorDemoTabs() {
  ...
  return (
    <div className={styles["layout-demo-tabs"]} role="tablist" aria-label="Layout indicator demo">
      {TABS.map((tab, i) => (
        <div
          key={tab}
          role="tab"
